...
try {
    ...
    & node $runtimeHelper stop electron "Mumbler" "Mumbler"
    if ($LASTEXITCODE -ne 0) { throw "Could not replace the existing Mumbler runtime." }
    ...
    if ($LASTEXITCODE -ne 0) { throw "Mumbler did not start." }
}
catch {
    Write-Host ""
