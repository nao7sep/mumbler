...
// and since Safari 27 so does focus a script moves after keyboard use. The startup
// window's body fell back to the browser's own ring that way.
//
...
const EXPLAINED: Record<string, { kind: Site["kind"]; reason: string }> = {
  "src/renderer/src/app/modal/focusTrap.ts": {
    kind: "attribute",
